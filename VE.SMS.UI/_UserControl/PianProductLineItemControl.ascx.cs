...
{
	public partial class PianProductLineItemControl : BaseUserControl
	{
		...
		private List<Product> GetProductList()
		{
			...
			var products = dal.SearchProduct(Code,
											ProductName,
											HuangCode,
											JiaCode,
											PianCode,
		...
		private double TotalSquare { get; set; }
		...
		protected void rpProductList_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			...
			{
				Product prd = e.Item.DataItem as Product;
				TotalSquare += prd.QuantityM2.HasValue ? prd.QuantityM2.Value : 0;
				TotalAmount += (prd.PriceM2.HasValue ? prd.PriceM2.Value : 0) * (prd.QuantityM2.HasValue ? prd.QuantityM2.Value : 0);
			}
			if (e.Item.ItemType == ListItemType.Footer)
			{
				...
				lblTotalSquare.Text = Utility.Round(TotalSquare).ToString();
				lblTotalAmount.Text = Utility.Round(TotalAmount).ToString();
			}
		}
